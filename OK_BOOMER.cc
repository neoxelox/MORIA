...
#define TROLL_WEIGHT_DWARF 99
#define SAFE_AREA_DWARF 99

#define DWARF_NOWNED_WEIGHT_WIZARD 999
#define WIZARD_NOWNED_WEIGHT_WIZARD 9
#define ORC_WEIGHT_WIZARD 999
#define TROLL_WEIGHT_WIZARD 999
#define SAFE_AREA_WIZARD 99
...
#define MAX_ENEMY_CELLS 50
  ...
  Pos findDwarf(Pos current_pos, int health) {
    clearMB(VisitedPos);
  ...
  void updateDwarvesWeightMap(MW &M, bool safeArea) {
    clsMW(M);
    Cell c; Pos p, mvp; Unit u; WeightPos wp; int w;
    for (p.i = 0; p.i < ROWS; p.i++) {
      for (p.j = 0; p.j < COLS; p.j++) {
        ...
        if (c.id != -1) {
          ...
          if (u.player != me()) {
            ...
            case Dwarf:
              w += (u.health/30);
              if (safeArea) {
                for (Dir mov : dwarvesDirs) {
                  mvp = p + mov;
                  if(pos_ok(mvp) && M[mvp.i][mvp.j].w < SAFE_AREA_DWARF) M[mvp.i][mvp.j].w = SAFE_AREA_DWARF;
                }
              }
              break;
            case Wizard:
            ...
            case Balrog:
              w += BALROG_WEIGHT;
              for (int k = 0; k < 25; k++) {
                mvp.i = p.i + balrogRadiusi[k];
                mvp.j = p.j + balrogRadiusj[k];
                if(pos_ok(mvp)) M[mvp.i][mvp.j].w = BALROG_WEIGHT;
              }
              break;
            default:
              w += NONE_WEIGHT;
        ...
        wp.p = p;
        wp.w = w;
        if (M[p.i][p.j].w < w) M[p.i][p.j] = wp;
      }
    }
  }

  void updateWizardsWeightMap(MW &M) {
    clsMW(M);
    Cell c; Pos p, mvp; Unit u; WeightPos wp; int w;
    for (p.i = 0; p.i < ROWS; p.i++) {
      for (p.j = 0; p.j < COLS; p.j++) {
        ...
        if (c.id != -1) {
          ...
          if (u.player != me()) {
            switch (u.type) {
            case Dwarf:
              w += DWARF_NOWNED_WEIGHT_WIZARD;
              for (Dir mov : dwarvesDirs) {
                  mvp = p + mov;
                  if(pos_ok(mvp) && M[mvp.i][mvp.j].w < DWARF_NOWNED_WEIGHT_WIZARD) M[mvp.i][mvp.j].w = DWARF_NOWNED_WEIGHT_WIZARD;
              }
              break;
            case Wizard:
              w += WIZARD_NOWNED_WEIGHT_WIZARD;
              break;
            case Orc:
              w += ORC_WEIGHT_WIZARD;
              for (Dir mov : dwarvesDirs) {
                  mvp = p + mov;
                  if(pos_ok(mvp) && M[mvp.i][mvp.j].w < ORC_WEIGHT_WIZARD) M[mvp.i][mvp.j].w = ORC_WEIGHT_WIZARD;
              }
              break;
            case Troll:
              w += TROLL_WEIGHT_WIZARD;
              for (Dir mov : dwarvesDirs) {
                  mvp = p + mov;
                  if(pos_ok(mvp) && M[mvp.i][mvp.j].w < TROLL_WEIGHT_WIZARD) M[mvp.i][mvp.j].w = TROLL_WEIGHT_WIZARD;
              }
              break;
            case Balrog:
              w += BALROG_WEIGHT;
  ...
  VP findShortestRegularPath(Pos start_pos, Pos final_pos) {
    ...
    while (pq.size() > 0) {
      ...
      if (VisitedPos[cwp.p.i][cwp.p.j]) continue;
      ...
      for (Dir movd : dwarvesDirs) {
        ctl = cwp.p + movd;
        if (!pos_ok(ctl)) continue;
  ...
  void setup() {
    ...
    Dwarves = VU();
    dwarvesDirs = VD{TL, Top, RT, Left, Right, LB, Bottom, BR};
    wizardDirs = VD{Top, Left, Right, Bottom};
    balrogRadiusi = {-1, -1, -1,  0,  0,  1,  1,  1, -2, -2, -2, -2, -2, -1, -1,  0,  0,  1,  1,  2,  2,  2,  2,  2};
    balrogRadiusj = {-1,  0,  1, -1,  1, -1,  0,  1, -2, -1,  0,  1,  2, -2,  2, -2,  2, -2,  2, -2, -1,  0,  1,  2};
  ...
  void moveWizards() {
    ...
    for (Unit Wizard : wizardsUnits) {
      VP pat = findShortestNonRegularPath(Wizard.pos, findDwarf(Wizard.pos,-1));
  ...
  virtual void play() {
    ...
    updateMaxEnemyInfo();
    updateDwarvesWeightMap(WeightMap, false);
    updateWizardsWeightMap(WeightMapWizard);  
